...
- `src/lib/signalGroups.ts` (generated by `scripts/mineGroups.ts`): mined strategy GROUPS whose joint BUY (all members same bar) hit 82-90% win rate over 5y + ≥70% on an 18-month holdout. Re-mine: `scripts/logFires.ts --chunk k/8` (8 parallel, ~45 min; logs fire-sets + outcomes to scratchpad) then `scripts/mineGroups.ts [--bar 80]`. recommend.ts evaluates combo members (incl. strategies not individually served) and attaches active combos; UI shows an amber comboHits section; ledger logs combo picks with comboId.
- Picks trade plan = the backtested multiples: target +1.5×ATR14, stop −2.5×ATR14, max hold 21 trading days (NOT the bot brain's 3×/2× thesis levels — keep these aligned or the track record grades the wrong plan).
- `src/lib/pickLedger.ts` + `/api/scoreboard`: the public track record. Full-universe pick runs append to `data/picks-ledger-{market}.json` (append-only; open symbol not re-added). Resolution: fill at next session open, exit at published target/stop (gap-aware after day 0, both-hit-same-day = loss) or 21-trading-day time exit; fills gapped through stop/target are `void` and excluded from stats. Lazy resolve on GET if >6h stale; cron forces resolve after each picks refresh. UI: Picks tab → Track Record toggle.
