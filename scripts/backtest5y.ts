...
`;
  ...
  // Metadata catalog of the SERVED strategies for the client bundle (the UI
  // can't import strategies.ts — that would ship every evaluate() to the
  // browser). Sorted by 5y win rate, best first.
  const byId = new Map(ALL_STRATEGIES.map((s) => [s.id, s]));
  const catalogEntries = passed
    .map((r: any) => ({ r, s: byId.get(r.id)! }))
    .filter((x: any) => x.s)
    .map(({ r, s }: any) =>
      `  { id: ${JSON.stringify(s.id)}, name: ${JSON.stringify(s.name)}, chapter: ${JSON.stringify(s.chapter)}, category: ${JSON.stringify(s.category)}, book: ${JSON.stringify(s.book)}, description: ${JSON.stringify(s.description)}, indicators: ${JSON.stringify(s.indicators)} },`
    )
    .join("\n");
  const catalog = `// AUTO-GENERATED by scripts/backtest5y.ts — do not edit by hand.
// Metadata for the ${passed.length} strategies the site serves (see
// verifiedStrategies.ts for the verification bar). Sorted by 5y win rate.
...
`;
  fs.writeFileSync(path.join(process.cwd(), "src", "lib", "strategyCatalog.ts"), catalog);
