...
export default function CandlestickChart({
  ...
  return (
    <div className="bg-white/[0.02] border border-white/5 rounded-xl overflow-hidden">
      ...
      {/* Timeframe tabs + Indicator toggles — horizontal scroll on mobile, wrap on desktop */}
      <div className="flex sm:flex-wrap items-center gap-1.5 px-4 py-2.5 border-b border-white/5 overflow-x-auto scrollbar-thin">
        {/* Timeframes */}
        <div className="flex items-center gap-0.5 bg-white/[0.03] rounded-lg p-0.5 mr-2 flex-shrink-0">
          {TIMEFRAMES.map((tf) => (
        ...
        {INDICATOR_OPTIONS.map((opt) => (
          <span key={opt.id} className="relative group flex-shrink-0">
            <button
              onClick={() => toggleIndicator(opt.id)}
              className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-all border whitespace-nowrap ${
